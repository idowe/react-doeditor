...
var EditorSelection = {
  ...
  getParagraphs: function() {
    ...
    for (var i = 0; i < textNodes.length; i++) {
      ...
      var tagName =
        textNodes[i].childNode.parentNode.tagName ||
        textNodes[i].childNode.parentNode.nodeName;
  ...
  addRange: function(startContainer, startOffset, endContainer, endOffset) {
    // addRange
    this.selection = this.getSelection();
    this.selection.removeAllRanges();
    if (
  ...
  validateSelection: function(selection) {
    ...
    if (
      anchorNode.offsetParent &&
      anchorNode.offsetParent.className == "editor-toolbar"
    )
      return false;
    if (
      parentNode.offsetParent &&
      parentNode.offsetParent.className == "editor-toolbar"
    )
      return false;
    return parentNode;
  },
  ...
  getRangeState: function() {
    ...
    // init icons state
    var canActiveIcons =
      "bold italic underline strikethrough fontborder superscript subscript justifycenter justifyleft justifyright justifyjustify";
    ...
    if (this.range) {
      ...
      while (
        (parentElement.tagName || parentElement.nodeName).toUpperCase() != "DIV"
      ) {
        ...
        switch (tagName.toUpperCase()) {
          ...
          case "H5":
          case "H6":
            var textAlign = parentElement.style.textAlign
              ? parentElement.style.textAlign
              : "left";
            var fontFamily = parentElement.style.fontFamily
              ? parentElement.style.fontFamily
              : "宋体,SimSun";
            var fontSize = parentElement.style.fontSize
    ...
    if (!rangeState["forecolor"])
      rangeState["forecolor"] = { color: "transparent", icon: "forecolor" };
    if (!rangeState["backcolor"])
      rangeState["backcolor"] = { color: "transparent", icon: "backcolor" };
    if (!rangeState["fontsize"] || !rangeState["fontsize"].value)
      rangeState["fontsize"] = { value: "3", icon: "fontsize" };
    if (!rangeState["paragraph"] || !rangeState["paragraph"].value)
      rangeState["paragraph"] = { value: "p", icon: "fontsize" };
    if (!rangeState["fontfamily"] || !rangeState["fontfamily"].value)
      rangeState["fontfamily"] = { value: "宋体, SimSun", icon: "fontfamily" };
